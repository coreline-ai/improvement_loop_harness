...
export function buildAdversaryLiveReviewerPrompt(input) {
  ...
  return [
    ...
    'The selected patch changes src/cart.cjs lineTotal from returning item.price to item.price * (item.quantity ?? 1).',
    'A useful proposal should exercise cart quantity semantics, including quantity > 1 and quantity: 0, so the buggy base fails and the selected candidate passes.',
    'The test body should require "../../src/cart.cjs" from tests/adversary/ and exit nonzero on mismatch.',
    '',
...
async function runCodexReviewer(prompt, options = {}) {
  const tmp = await mkdtemp(path.join(os.tmpdir(), 'adversary-live-codex-reviewer-'));
