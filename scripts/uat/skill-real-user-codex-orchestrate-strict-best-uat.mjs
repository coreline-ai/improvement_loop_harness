#!/usr/bin/env node
// Real-user Codex LIVE orchestrate RU-3 strict-best UAT.
//
// Purpose: prove the automatic-discovery path can also make a fixed-score
// best-fix choice across multiple issues. Unlike plain RU-3, this run uses an
// intentionally verbose deterministic command builder as an accepted-but-worse
// comparator and a real Codex challenger for every discovered issue.
//
// A FULL improvement PASS requires all of these to be true:
//   1. automatic discovery finds each issue from current repo state,
//   2. each issue gets a task/eval from discovery evidence,
//   3. verbose comparator and real Codex challenger both enter the candidate set,
//   4. the selected patch is final-reverified/provenance-checked,
//   5. the selected patch is committed, rediscovered, and published as stacked
//      GitHub draft PRs,
//   6. every issue reports `selection_quality.strict_score_improvement=true`.
//
// Honest scope: this is the first broad multi-issue strict-best proof lane. If
// any issue lacks strict fixed-score improvement, the script may still prove
// RU-3 verification but MUST NOT claim full autonomous improvement.
...
const maxCandidates = Number(process.env.VIBELOOP_UAT_RU3_MAX_CANDIDATES || '2');
...
async function writeVerboseComparatorAgent(file) {
  await writeFile(
    file,
    [
      '#!/usr/bin/env node',
      ...
      "if (!taskFile) throw new Error('missing VIBELOOP_TASK_FILE');",
      "const task = JSON.parse(fs.readFileSync(taskFile, 'utf8'));",
      "const target = task.write_scope?.allowed?.[0] || '';",
      "if (target === 'src/cart-total.cjs') {",
      "  fs.writeFileSync(target, [",
      "    'function calculateTotal(items) {',",
      "    '  const safeItems = Array.isArray(items) ? items : [];',",
      "    '  return safeItems.reduce((runningTotal, item) => {',",
      ...
      "    ''",
      "  ].join('\\n'));",
      "  console.log('verbose comparator fixed cart-total');",
      "} else if (target === 'src/sku.cjs') {",
      "  fs.writeFileSync(target, [",
      ...
      "    ''",
      "  ].join('\\n'));",
      "  console.log('verbose comparator fixed sku');",
      "} else {",
      "  throw new Error(`unsupported strict-best comparator target: ${target}`);",
      "}",
      ''
    ].join('\n'),
    { mode: 0o755 }
...
function issueStrictStatus(issue) {
  ...
  return {
    ...
    selected_candidate_id: issue.selected_candidate_id ?? null,
    selected_is_real_codex_challenger:
      typeof issue.selected_candidate_id === 'string' &&
      issue.selected_candidate_id.endsWith('-c1'),
...
async function main() {
  ...
  const repoName = `vibeloop-realuser-ru3-strict-${tag}`;
  const fullRepo = `${owner}/${repoName}`;
  const tmpRoot = await mkdtemp(path.join(os.homedir(), '.vibeloop-realuser-ru3-strict-'));
  ...
  try {
    ...
    await git(localRepo, ['commit', '-m', 'seed: ru3 strict-best auto-discovery bugs + failing tests']);
    await writeVerboseComparatorAgent(verboseAgent);
    ...
    const codexAgentSpec = buildCodexOAuthCommand({
    ...
    const cli = await run(
      ...
      [
        ...
        '--promote-branch',
        'vibeloop/ru3-strict-integration',
        '--promote-commit-message-prefix',
        'vibeloop ru3 strict selected patch',
        '--github-draft-pr',
        '--github-repo',
        fullRepo,
        '--github-token-env',
    ...
    await git(localRepo, ['checkout', 'vibeloop/ru3-strict-integration']);
    ...
    const realCodexSelectedEveryIssue =
      issues.length === maxIssues &&
      issueStatus.every((issue) => issue.selected_is_real_codex_challenger);
    ...
    const fullAutonomousImprovementPass =
      verificationPass && strictImprovementEveryIssue && realCodexSelectedEveryIssue;

    const ledger = {
      status: !verificationPass
        ? 'REAL_USER_RU3_STRICT_BEST_FAIL'
        : fullAutonomousImprovementPass
          ? 'REAL_USER_RU3_FULL_IMPROVEMENT_PASS'
          : 'REAL_USER_RU3_ORCHESTRATE_VERIFICATION_PASS_BEST_UNPROVEN',
      scenario: 'skill-real-user-codex-orchestrate-strict-best-uat',
      mode: 'auto-discovery orchestrate with verbose comparator + real Codex challenger + GitHub draft PRs',
      orchestrator:
        'vibeloop orchestrate --generate-eval --promote-branch --github-draft-pr --challenger',
      ...
      limitations: fullAutonomousImprovementPass
        ? [
            'proves broad multi-issue strict fixed-score improvement for this controlled RU-3 fixture',
            'generated eval is visible-test/minimal; hidden/adversary semantic eval generation remains future work'
          ]
        : [
            'may prove RU-3 verification only; does not prove full autonomous improvement unless full_autonomous_improvement_pass is true',
            'generated eval is visible-test/minimal; hidden/adversary semantic eval generation remains future work'
          ]
    };

    if (
      ledger.status === 'REAL_USER_RU3_FULL_IMPROVEMENT_PASS' &&
      (!strictImprovementEveryIssue || !realCodexSelectedEveryIssue)
    ) {
    ...
    if (
      ...
      throw new Error('strict full-pass invariant violated: PASS status requires full_autonomous_improvement_pass=true');
    }
    if (
      ledger.full_autonomous_improvement_pass !==
      (verificationPass && strictImprovementEveryIssue && realCodexSelectedEveryIssue)
      ...
      throw new Error(
        'strict full-pass invariant violated: full_autonomous_improvement_pass must equal verificationPass && strictImprovementEveryIssue && realCodexSelectedEveryIssue'
      );
    }
    assertNoHiddenLeak('ledger', ledger);
    console.log(JSON.stringify(ledger, null, 2));
    if (ledger.status !== 'REAL_USER_RU3_FULL_IMPROVEMENT_PASS') {
      process.exitCode = 1;
    }
  } finally {
