...
import { buildPullRequestBody, createDraftPullRequest, GitHubApiError, parseGitHubRepo } from './pull-request.js';
...
describe('GitHub draft PR integration', () => {
  ...
  it('parses GitHub repository URLs without accepting non-GitHub remotes', () => {
    expect(parseGitHubRepo('https://github.com/coreline-ai/improvement_loop_harness.git')).toEqual({
      owner: 'coreline-ai',
      repo: 'improvement_loop_harness'
    });
    expect(parseGitHubRepo('git@github.com:coreline-ai/improvement_loop_harness.git')).toEqual({
      owner: 'coreline-ai',
      repo: 'improvement_loop_harness'
    });
  ...
  it('creates and pushes a branch from candidate.patch with safe git', async () => {
    ...
    const patchPath = path.join(await tempDir('vibeloop-github-patch-'), 'candidate.patch');
    await writeFile(
      patchPath,
      ['diff --git a/README.md b/README.md', '--- a/README.md', '+++ b/README.md', '@@ -1 +1,2 @@', ' # fixture repo', '+patched', ''].join('\n')
    );
    ...
    const remoteHead = (await repo.git(['ls-remote', bareRemote, branch.branchName])).trim();
  ...
  it('reuses an existing open PR and does not create duplicates on retry', async () => {
    nock('https://api.github.com')
      .get('/repos/coreline-ai/improvement_loop_harness/pulls')
      .query({ head: 'coreline-ai:vibeloop/loop-1', state: 'open' })
      .reply(200, [{ html_url: 'https://github.com/coreline-ai/improvement_loop_harness/pull/8', number: 8 }]);
