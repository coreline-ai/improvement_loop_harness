...
describe('schema registry', () => {
  ...
  it('includes at least three valid and invalid YAML fixtures for Phase 2', async () => {
    await expect(
      readdir(path.join(fixturesRoot, 'valid'))
  ...
  it('reports schema instance paths when validation fails', () => {
    try {
      ...
      expect(
        (error as SchemaValidationError).details.some((detail) =>
          detail.startsWith('/ ')
        )
      ).toBe(true);
      expect((error as SchemaValidationError).details.join(' ')).toContain(
        "required property 'write_scope'"
      );
    }
  });

  it('requires execution.image when execution.isolation is container (R1)', () => {
    const base = {
      schema_version: '1.0',
      project: 'exec-iso',
      gates: [
        { name: 'g', type: 'task_acceptance', command: 'true', required: true }
      ]
    };
    expect(() =>
      validateOrThrow(
        EVAL_SCHEMA_ID,
        { ...base, execution: { isolation: 'container' } },
        'exec-no-image'
      )
    ...
    expect(() =>
      validateOrThrow(
        ...
        {
          ...base,
          execution: { isolation: 'container', image: 'alpine:3.20' }
        },
        'exec-with-image'
      )
    ).not.toThrow();
...
describe('loadTask', () => {
  it('loads the TASK_PROTOCOL §2 task fixture and normalizes repo-relative paths', async () => {
    ...
    expect(task.write_scope.allowed).toEqual([
      'src/features/auth/',
      'src/app/api/auth/',
      'tests/auth/'
    ]);
  ...
  it('rejects absolute and parent traversal write scope paths', async () => {
    await expect(
      loadTask(fixturePath('invalid', 'task-absolute-path.yaml'))
    ).rejects.toThrow(TaskProtocolError);
    await expect(
      loadTask(fixturePath('invalid', 'task-parent-path.yaml'))
    ).rejects.toThrow(TaskProtocolError);
  });
});
...
describe('loadEvalConfig', () => {
  it('loads the EVAL_ENGINE_SPEC §10 eval fixture and preserves gate order', async () => {
    const config = await loadEvalConfig(
      fixturePath('valid', 'eval-valid.yaml')
    );
    ...
    expect(config.risk_classification?.auth).toEqual([
      'src/features/auth/',
      'src/app/api/auth/'
    ]);
  });
  ...
  it('rejects project command gates before scope/integrity guard gates', async () => {
    await expect(
      loadEvalConfig(fixturePath('invalid', 'eval-order-violation.yaml'))
    ).rejects.toThrow(EvalConfigError);
  });

  it('rejects unsupported interpolation variables in gate commands', async () => {
    await expect(
      loadEvalConfig(fixturePath('invalid', 'eval-unknown-var.yaml'))
    ).rejects.toThrow(InterpolationError);
  });
});
...
describe('limits, interpolation, and risk helpers', () => {
  it('mergeLimits keeps the stricter numeric values', () => {
    expect(
      mergeLimits({ max_changed_lines: 300 }, { max_changed_lines: 500 })
    ).toEqual({
  ...
  it('allows only the five documented interpolation variables', () => {
    expect(() =>
      assertAllowedInterpolation('npm test -- ${TASK_FILE} ${BASE_COMMIT}')
    ).not.toThrow();
    expect(() => assertAllowedInterpolation('npm test -- ${UNKNOWN}')).toThrow(
      InterpolationError
    );
    expect(
