...
- ✅/⚠️ **adversary rulepack substrate** — M2 handoff→candidate→M4 replay/freeze→`rulepack_semantic` next-loop gate core와 `rulepack inspect`, `uat:adversary-live` 드라이버는 구현. CI `adversary-live` job은 Docker R1에서 evidence artifact를 남기도록 구성됐고, `release-evidence-audit` job이 해당 artifact의 attack/safety/reviewer provenance ledger를 검증하도록 연결됐다. 기본 controlled lane은 `real_llm=false`로만 인정되며, optional reviewer command lane은 `real_llm=true`, `accepted_review_proposal`, fixed prompt hash/version, `same_model_review=false`가 있어야 release-grade evidence로 통과한다. P4 required attack scenario는 test weakening, hidden artifact leak, prompt injection, visible-only hardcode, default-quantity hardcode, zero-quantity truthiness hardcode 6종이며, semantic 3종은 N+1 `rulepack_semantic` executed=true 차단을 요구한다. 2026-06-22 local run `adversary-live-39896-1782094414571`은 `node scripts/uat/adversary-live-codex-reviewer.mjs --live` 기반 real Codex reviewer proposal(`cart-line-total-quantity-semantics`)을 받아 M2/M4/freeze/N+1과 6/6 attack scenario를 통과했다. 별도 `adversary-live-real-reviewer-uat` scenario와 `P4 Real Reviewer Live Evidence` workflow는 real-reviewer CI artifact 감사를 위해 추가됐다. 단 이 증거는 아직 단일 P4 cart semantic lane 중심이며, 더 큰 project-specific semantic/M4 corpus와 실제 CI real-reviewer artifact PASS는 후속이다.
...
- 🚧 **남은 작업** — Product-100 controlled corpus 밖의 임의/대형 실제 프로젝트 corpus, 더 큰 project-specific semantic/M4 corpus, `P4 Real Reviewer Live Evidence` workflow의 실제 credentialed CI PASS, CI artifact 기반 장기 감사.
