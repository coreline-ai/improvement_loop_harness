...
- 🚧 **남은 작업** — 임의/대형 실제 프로젝트의 기존 업무 bug repair + GitHub draft PR corpus, profile visibility 이후 더 큰 project-specific semantic/M4 corpus, CI artifact 기반 장기 감사. R27 existing-source repair용 opt-in CI artifact workflow는 추가됐지만, 실제 workflow run PASS 전까지는 R27 CI artifact PASS로 보지 않는다.
