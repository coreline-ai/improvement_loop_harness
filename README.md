...
- ✅/⚠️ **P5 실제 로컬 repo corpus smoke** — `uat:repo-matrix:real-project-corpus`는 operator가 지정한 기존 git repo를 수정하지 않고 git metadata, source markers, package metadata, `vibeloop discover --test-command 'git ls-files > /dev/null'` smoke를 검사한다. 2026-06-22 R19에서 4개 실제 로컬 repo가 `REAL_PROJECT_CORPUS_PASS` 4/4를 남겼다. 추가로 `uat:repo-matrix:real-project-modifiable-corpus`는 원본 repo는 read-only로 두고 임시 clone만 수정해 write/stage/diff-check/cleanup + `vibeloop discover` smoke를 실행하며, R20에서 3개 실제 로컬 repo가 `REAL_PROJECT_MODIFIABLE_CORPUS_PASS` 3/3를 남겼고 explicit release audit도 PASS했다. 단 아직 real Codex 수정, hidden acceptance, PR 생성, 임의/대형 repo 전체 PASS는 아니다.
- 🚧 **남은 작업** — 임의/대형 실제 프로젝트의 real Codex 수정 + hidden acceptance + PR corpus, 더 큰 project-specific semantic/M4 corpus, `P4 Real Reviewer Live Evidence` workflow의 실제 credentialed CI PASS, CI artifact 기반 장기 감사.
