...
- ✅/⚠️ **adversary rulepack substrate** — M2 handoff→candidate→M4 replay/freeze→`rulepack_semantic` next-loop gate core와 `rulepack inspect`, `uat:adversary-live` 드라이버는 구현. CI `adversary-live` job은 Docker R1에서 evidence artifact를 남기도록 구성됐고, `release-evidence-audit` job이 해당 artifact의 attack/safety/reviewer provenance ledger를 검증하도록 연결됐다. 기본 controlled lane은 `real_llm=false`로만 인정되며, optional reviewer command lane은 `real_llm=true`, `accepted_review_proposal`, fixed prompt hash/version, `same_model_review=false`가 있어야 release-grade evidence로 통과한다. P4 required attack scenario는 test weakening, hidden artifact leak, prompt injection, visible-only hardcode, default-quantity hardcode, zero-quantity truthiness hardcode, discount hardcode 7종이며, semantic 4종은 N+1 `rulepack_semantic` executed=true 차단을 요구한다. 2026-06-22 local controlled run `adversary-live-13113-1782097233594`는 quantity+discount 2-rule M4 corpus(`case_count=2`)와 7/7 attack scenario를 통과했다. 별도 `adversary-live-real-reviewer-uat` run `adversary-live-real-reviewer-14014-1782097278557`도 real Codex reviewer proposal + supplemental discount semantic rule을 받아 `real_llm=true`, M2 confirmed 2, M4 replay-safe 2/2, N+1 7/7 PASS를 남겼다. 단 실제 CI real-reviewer artifact PASS와 더 큰 project-specific semantic/M4 corpus는 후속이다.
...
- ✅/⚠️ **P5 실제 로컬 repo read-only corpus smoke** — `uat:repo-matrix:real-project-corpus`는 operator가 지정한 기존 git repo를 수정하지 않고 git metadata, source markers, package metadata, `vibeloop discover --test-command 'git ls-files > /dev/null'` smoke를 검사한다. 2026-06-22 R19에서 4개 실제 로컬 repo가 `REAL_PROJECT_CORPUS_PASS` 4/4를 남겼고, `release-evidence-audit --scenario repo-matrix-real-project-corpus-uat`가 해당 manifest-backed evidence를 감사했다. 단 read-only smoke라 LLM 수정, hidden acceptance, PR 생성, 임의/대형 repo 전체 PASS는 아니다.
- 🚧 **남은 작업** — read-only smoke를 넘는 임의/대형 실제 프로젝트 수정 corpus, 더 큰 project-specific semantic/M4 corpus, `P4 Real Reviewer Live Evidence` workflow의 실제 credentialed CI PASS, CI artifact 기반 장기 감사.
