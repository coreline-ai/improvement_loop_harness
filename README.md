...
corepack pnpm uat:repo-matrix:real-project-semantic-source-repair -- --repo <repo> --repo <repo> # P5 실제 repo curated semantic 기존 source repair + hidden verifier
...
| R35         | 실제 repo semantic 기존 source repair 2셀          | real Codex semantic source repair           | PASS — current repo `improvement_loop_harness` + public `pypa/sampleproject` 2셀에서 curated semantic target(`scripts/uat/product-100-corpus.mjs` issue count summary, `src/sample/simple.py` add_one)을 behavioral regression으로 깨뜨린 뒤, 실제 Codex가 기존 tracked source file만 수정해 original visible/hidden pass → regressed visible/hidden expected fail → final visible/hidden pass, existing-source-only diff, source repo integrity pass를 남겼다. Ledger `real-project-corpus-36826-1782130475791`, scenario `repo-matrix-real-project-semantic-source-repair-uat`, `REAL_PROJECT_SEMANTIC_SOURCE_REPAIR_PASS`, `cell_count=2`, `pass_count=2`, `fail_count=0`, explicit release audit PASS. 단 curated semantic target registry 범위이며, 임의 업무 source bug repair·GitHub draft PR·임의/대형 repo 전체 PASS는 아니다.                                                                                         |
...
- ✅/⚠️ **P5 실제 로컬 repo corpus smoke/repair** — `uat:repo-matrix:real-project-corpus`는 operator가 지정한 기존 git repo를 수정하지 않고 git metadata, source markers, package metadata, `vibeloop discover --test-command 'git ls-files > /dev/null'` smoke를 검사한다. 2026-06-22 R19에서 4개 실제 로컬 repo가 `REAL_PROJECT_CORPUS_PASS` 4/4를 남겼다. `uat:repo-matrix:real-project-modifiable-corpus`는 원본 repo는 read-only로 두고 임시 clone만 수정해 write/stage/diff-check/cleanup + `vibeloop discover` smoke를 실행하며, R20에서 3개 실제 로컬 repo가 `REAL_PROJECT_MODIFIABLE_CORPUS_PASS` 3/3를 남겼다. `uat:repo-matrix:real-project-codex-copy`는 원본 repo는 read-only로 두고 임시 clone에서 실제 Codex가 probe JSON만 작성하게 한 뒤 hidden verifier가 repo-derived head/file-count/diff scope를 확인하며, R22에서 2개 실제 로컬 repo가 `REAL_PROJECT_CODEX_COPY_PASS` 2/2와 explicit release audit PASS를 남겼다. `uat:repo-matrix:real-project-codex-repair`는 같은 2개 실제 로컬 repo의 temp clone에 dedicated fixture source/test commit을 만들고 실제 Codex가 source file만 repair하게 한 뒤 base fail, final visible/hidden pass, source-only diff, 원본 repo integrity를 검증하며, R24에서 `REAL_PROJECT_CODEX_REPAIR_PASS` 2/2와 explicit release audit PASS를 남겼다. `uat:repo-matrix:real-project-business-repair`는 실제 로컬 repo 2개(`html-skills-doc`, `coreline-auth-module`)의 temp clone에 invoice-total business logic fixture를 넣고 실제 Codex가 quantity/discount/tax/final-rounding semantics를 source-only로 고치게 하며, R32에서 `REAL_PROJECT_BUSINESS_REPAIR_PASS` 2/2와 explicit release audit PASS를 남겼다. R33에서는 같은 business fixture repair lane을 `Real Project Repair Evidence` workflow의 credentialed self-hosted Codex runner artifact로 재현해 live job과 downloaded artifact audit이 모두 PASS했다. `uat:repo-matrix:real-project-existing-source-repair`는 기존 tracked JS/Python source file에 syntactic regression을 넣고 실제 Codex가 그 기존 source file만 복구해야 하며, R27에서 2개 실제 로컬 repo가 `REAL_PROJECT_EXISTING_SOURCE_REPAIR_PASS` 2/2와 explicit release audit PASS를 남겼다. R29에서는 public 실제 repo 4개(`pypa/sampleproject`, `pallets/click`, `expressjs/express`, `nodeca/js-yaml`)로 같은 existing-source repair lane을 넓혀 `REAL_PROJECT_EXISTING_SOURCE_REPAIR_PASS` 4/4와 explicit release audit PASS를 남겼고, credentialed CI run `27947984264`에서도 same 4-repo artifact + downloaded audit이 PASS했다. `uat:repo-matrix:real-project-existing-source-repair-pr`는 같은 existing-source repair를 GitHub private evidence repo + draft PR publish까지 확장하며, R28에서 2개 실제 repo가 `REAL_PROJECT_EXISTING_SOURCE_REPAIR_PR_PASS` 2/2와 explicit release audit PASS를 남겼고, credentialed CI run `27949790976`에서도 current repo + `pypa/sampleproject` 2-cell draft PR artifact + downloaded audit이 PASS했다. `uat:repo-matrix:real-project-semantic-source-repair`는 curated semantic target registry의 기존 source behavior regression을 실제 Codex가 복구해야 하며, R35에서 current repo + `pypa/sampleproject` 2셀 `REAL_PROJECT_SEMANTIC_SOURCE_REPAIR_PASS`와 explicit release audit PASS를 남겼다. `Real Project Repair Evidence` workflow의 `repair_mode=semantic-source`는 같은 R35 lane을 credentialed CI artifact로 재현하도록 배선됐다. 단 R32/R33도 dedicated business fixture repair이고, R28/R29는 syntactic regression repair smoke이며, R35는 curated semantic target repair smoke다. 기존 애플리케이션 업무 소스의 임의 버그 repair나 임의/대형 repo 전체 PASS는 아니다.
- 🚧 **남은 작업** — 임의/대형 실제 프로젝트의 기존 애플리케이션 업무 source bug repair corpus, profile suspension 이후 더 큰 project-specific semantic/M4 corpus, CI artifact 기반 장기 감사. R32 business fixture repair와 R27/R28/R29 existing-source syntactic repair는 PASS했지만, 기존 앱 업무 source의 임의 bug repair/PR 검증과 더 큰 project-specific semantic/M4 corpus는 아직 후속이다.
