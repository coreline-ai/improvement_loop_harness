...
| Product-100 | 풀 자율루프 UAT 계약/하네스 | real Codex Builder/Challenger/Reviewer 필수 | **PARTIAL LIVE / PASS 아님** — 20개 fixed requirement 계약·10이슈 corpus·hidden eval generator·strict-best·Phase6 audit 계약은 구현. R1/reviewer preflight는 통과했고, 2개 issue smoke에서 real Codex 후보 2개씩→hidden/reverify→strict-best→rediscovery PASS를 실측. Phase5 real reviewer는 반례를 제안했고 M2 R1에서 `base=fail candidate=fail`로 더 개선 필요를 확인했다. 그 반례를 next-loop repair task/eval/test로 materialize하고 Builder/Challenger repair loop로 재수정·commit되는 경로를 구현했다. `NM-001` fresh one-shot smoke에서는 current-code Phase5 driver 1회 호출로 real Codex repair loop가 strict-best repair patch를 선택·commit했고, post-repair M2/M4/freeze/N+1 semantic gate까지 자동 연쇄 PASS했다. 단 10개 전체 issue 실측, 모든 이슈 adversary repair 실측, Phase6 GitHub draft PR live audit 미완. 코드상 Phase5는 모든 issue aggregate runner로 승격됐고, Phase6도 expected issue 전체 PR/evidence aggregate guard와 runner가 추가됐으며, Phase7 docs/Run Ledger/README truth checker도 결정론 runner로 추가됨 |
...
- 🚧 **Product-100 Codex Live UAT** — `uat:product-100:*` 계약/하네스가 추가됐다. `PRODUCT_100_CODEX_LIVE_PASS`는 20개 fixed requirement(`docs_run_ledger_readme_truthful` 포함)가 모두 true일 때만 가능하다. 현재 env에서는 기본 reviewer wrapper와 R1 Colima preflight가 통과한다. `VIBELOOP_PRODUCT_100_MAX_ISSUES=2` smoke는 2개 issue에서 real Codex 후보 2개씩, hidden acceptance, final reverify, strict-best, rediscovery까지 통과했다. Phase5 real reviewer smoke는 추가 edge proposal을 만들었고 M2 R1에서 현재 candidate도 실패하는 반례(`base=fail candidate=fail`)를 확인했다. 현재는 이 반례를 next-loop repair task/eval/test로 변환하고 Builder/Challenger repair loop에서 재수정·commit되는 경로까지 구현했다. repair agent env가 없을 때도 기본 Codex OAuth proxy 기반 repair Builder/Challenger agent set을 생성한다. 추가로 `NM-001` fresh one-shot smoke에서 current-code Phase5 driver가 real Codex repair loop와 post-repair M2/M4/freeze/N+1 semantic gate를 한 번에 자동 연쇄 PASS했다. Python 실행 이미지 mismatch를 수정하고 `PY-001` targeted live rerun은 hidden/reverify/strict-best/rediscovery까지 통과했다. 그러나 10개 전체 issue와 전체 adversary repair/Phase6 proof가 없어 전체 status는 여전히 `PRODUCT_100_CODEX_LIVE_FAIL`이어야 정상이다. 2026-06-18 추가 실측에서는 full run 중 RX-001의 `only_one_accepted_candidate` strict-best comparator 부족을 확인해 PASS 불가능 run을 중지했고, agent timeout/strict-best retry를 보강했다. 이후 RX-001 targeted live rerun은 hidden/reverify/strict-best/rediscovery까지 통과했지만 filtered 1/10 run이므로 제품 PASS가 아니다. 따라서 제품 전체 100% PASS가 아니라 **Product-100 partial live smoke가 통과한 상태**다.
...
- 🚧 **남은 작업** — Product-100 10개 전체 issue sequential run · 모든 issue에서 adversary 반례 repair loop 반복 · 전체 M4/freeze/N+1 semantic gate · 실제 GitHub draft PR Product-100 proof · Postgres release-grade evidence.
