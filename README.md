...
- ✅/⚠️ **adversary rulepack substrate** — M2 handoff→candidate→M4 replay/freeze→`rulepack_semantic` next-loop gate core와 `rulepack inspect`, `uat:adversary-live` 드라이버는 구현. CI `adversary-live` job은 Docker R1에서 evidence artifact를 남기도록 구성됐고, `release-evidence-audit` job이 해당 artifact의 attack/safety/reviewer provenance ledger를 검증하도록 연결됐다. 기본 controlled lane은 `real_llm=false`로만 인정되며, optional reviewer command lane은 `real_llm=true`, `accepted_review_proposal`, fixed prompt hash/version, `same_model_review=false`가 있어야 release-grade evidence로 통과한다. P4 required attack scenario는 test weakening, hidden artifact leak, prompt injection, visible-only hardcode, default-quantity hardcode, zero-quantity truthiness hardcode, discount hardcode, tax hardcode, rounding hardcode, profile visibility hardcode, profile suspension hardcode, order approval hardcode 12종이며, semantic 9종은 N+1 `rulepack_semantic` executed=true 차단을 요구한다. 2026-06-22 local controlled run `adversary-live-85538-1782133501519`는 quantity+discount+tax+rounding+profile visibility+profile suspension+order approval 7-rule M4 corpus(`case_count=7`)와 12/12 attack scenario를 통과했다. 별도 `adversary-live-real-reviewer-uat` local run `adversary-live-real-reviewer-85952-1782133535704`도 real Codex reviewer proposal + supplemental discount/tax/rounding/profile visibility/profile suspension/order approval semantic rule을 받아 `real_llm=true`, M2 confirmed 7, M4 replay-safe 7/7, N+1 12/12 PASS를 남겼다. Credentialed CI run `27943361464`는 이전 6-rule corpus에서 ledger `adversary-live-real-reviewer-86899-1782121115050`와 downloaded artifact audit PASS를 남겼다. M2는 proposal별 staged copy에서 실행되어 한 reviewer proposal의 test 파일이 다른 proposal 확인을 오염하지 않는다. 단 7-rule corpus의 credentialed CI artifact 재현성은 아직 후속이다.
