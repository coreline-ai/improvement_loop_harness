...
|    7 | agent stdout/stderr context·secret 누설 (forbidden literal 또는 opt-in token) | reject             | `GUARD_ARTIFACT_LEAK`          |
...
builder provider는 agent spec에서 도출한다(`codex*`→`openai`, `mock:*`→`mock`, `command:*`→`command`, 그 외→`unknown`). advisory finding에는 `builder_provider`/`reviewer_provider`도 함께 기록한다. 이 의미는 schema_version `1.1` 이후 신규 생성 report 기준이며, **advisory는 decision 14규칙에 참여하지 않는다.**
