...
- Docker-compatible runtime이 있으면 preflight는 `status=pass`, `uat:adversary-live`는 controlled command adversary proposal을 M2 격리 confirm → M4 replay → freeze → N+1 `builtin:rulepack-semantic` good/pass, bad/fail, visible-only hardcode/fail부터 payment method update hardcode/fail까지 project-specific semantic reject gate를 실행한다.
...
2026-06-23 local run `adversary-live-real-reviewer-18472-1782167542594`은 이 설정으로 `ADVERSARY_LIVE_PASS`를 남겼고, Codex reviewer proposal과 supplemental discount/tax/rounding/profile visibility/profile suspension/order approval/inventory reservation/shipping eligibility/payment authorization/refund eligibility/coupon application semantic rule을 M2/M4/freeze/N+1로 고정했다. Evidence는 `real_llm=true`, `provider=codex`, M2 confirmed 12, M4 replay-safe 12/12, 17/17 attack scenario PASS를 포함한다. M2는 proposal별 staged copy에서 실행되어 이전 proposal test 파일이 다음 proposal 확인을 오염하지 않는다. 2026-06-27 기본 `adversary-live-uat` controlled lane은 `adversary-live-61659-1782570018739`로 payment method update까지 포함한 48-rule corpus, M2 confirmed 48, M4 replay-safe 48/48, 53/53 attack scenario, explicit release audit 및 DB 포함 release-gates preflight PASS를 남겼다. CI run `28079085647` / ledger `adversary-live-2538-1782281588903`은 15-rule corpus, 20/20 attack scenario, artifact-bound release audit PASS 범위다. `adversary-live-real-reviewer-uat`는 real reviewer lane의 CI artifact 감사를 위한 별도 scenario이며, `release-evidence-audit --scenario adversary-live-real-reviewer-uat`는 real LLM reviewer provenance를 필수로 요구한다. 이전 6-rule corpus는 credentialed CI run `27943361464` / ledger `adversary-live-real-reviewer-86899-1782121115050`에서도 PASS했고, live job과 downloaded artifact audit job이 모두 성공했다. 48-rule real reviewer/credentialed CI artifact 재현성은 아직 후속이다.
...
현재 이 머신에서는 R1/reviewer preflight가 통과하고, 2026-06-23 finalization 기준 Product-100 controlled corpus ledger가 `PRODUCT_100_CODEX_LIVE_PASS`로 닫힌다. 핵심 증거는 final report `/Users/iriver/.vibeloop/product-100-real-loop-St3cnO/product-100-live-report.json`, final evidence bundle `~/.vibeloop/uat-evidence/product-100-codex-live-uat/product-100-83865-1782157755473`, GitHub draft PR 10개 `coreline-ai/vibeloop-p100-product-100-83865-1782157755473-*`다. Latest audit-backed full local/live Skill UX evidence는 R135이며 full `skill-full-83582-1782560947270` 27/27, matrix `skill-prompt-matrix-95082-1782560950802` 58/58, journey `skill-prompt-journey-95261-1782560954627` 3/3, prompt corpus `skill-prompt-corpus-live-96439-1782563498442` 48/48(user_issue 24/24, auto_discovery 24/24, real Codex `gpt-5.5`, false_pass 0, leak 0), local release evidence audit copied integrity checked 1223을 남겼다. R140은 기본 후보를 56-variant로 확장했고 copied Skill matrix `skill-prompt-matrix-5195-1782573069742` 66/66 및 신규 8개 targeted live `skill-prompt-corpus-live-91008-1782572277046` 8/8(user_issue 4/4, auto_discovery 4/4, real Codex `gpt-5.5`, false_pass 0, leak 0)을 임시 evidence root `/tmp/vibeloop-skill-new-prompts.XeiOTT`에 남겼다. 단 R135/R140은 GitHub PR publication 없는 bounded local/live Skill UX 증거라 56-variant full-live 묶음, 56-variant GitHub PR corpus, 임의 repo 전체 PASS가 아니다. P4 semantic/M4 latest local controlled lane은 R139 evidence `~/.vibeloop/uat-evidence/adversary-live-uat/adversary-live-61659-1782570018739`로 payment method update까지 포함한 48-rule corpus, M2 confirmed 48, M4 replay-safe 48/48, 53/53 attack scenario PASS를 남겼다. Latest artifact-bound replay는 R58 workflow run `28079085647`, ledger `adversary-live-2538-1782281588903`의 15-rule P4 corpus 범위다. Latest local real reviewer evidence는 R50 `~/.vibeloop/uat-evidence/adversary-live-real-reviewer-uat/adversary-live-real-reviewer-18472-1782167542594`이며 12-rule corpus, M2 confirmed 12, M4 replay-safe 12/12, 17/17 attack scenario PASS 기준이다. 48-rule real reviewer/credentialed artifact 재현성은 아직 후속이다. Broad real project corpus는 R49에서 public 실제 repo 8개 existing-source repair + hidden verifier, R28에서 GitHub draft PR smoke, R32/R33에서 business fixture repair, R48에서 public 실제 repo 12개 curated semantic existing-source repair + hidden verifier, R134에서 targeted business-source aggregate 24셀 PASS까지 통과했고, R136 referral reward targeted 1셀도 PASS했다. 단 R136 25셀 aggregate는 Codex usage limit로 5/25 FAIL했으므로 release gate 기준은 여전히 R134 24/24다. R28/R29/R34/R49는 syntactic regression repair smoke, R32/R33은 dedicated business fixture repair, R40/R41/R43/R45/R46/R48은 curated semantic target repair, R67~R136은 targeted business-source repair이므로, 기존 애플리케이션 업무 source의 임의 bug repair나 임의 사용자 repo 전체에 대한 제품 전체 100% PASS는 아니다.
