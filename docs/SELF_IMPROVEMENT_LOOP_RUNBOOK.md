...
기대: GitHub-hosted label은 `status=pass`, custom/self-hosted label은 matching online runner가 없으면 `status=blocked`, `reason=SELF_HOSTED_RUNNER_UNAVAILABLE`. GitHub Actions token이 runner 목록을 조회할 권한이 없으면 `reason=RUNNER_QUERY_TOKEN_UNAVAILABLE`로 차단한다. 두 credentialed workflow는 같은 preflight를 먼저 실행하고 `*-live-runner-preflight-*` artifact를 남기므로, runner/token 부재는 live evidence PASS가 아니라 환경 차단으로 해석한다.
