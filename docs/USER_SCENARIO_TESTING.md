...
| ---- | ------------------------------------------ | -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------ | ----------------------------------- |
| 1    | 로컬 command agent 사용자 시나리오         | mock 없이 실제 CLI가 임시 target repo를 만들고 파일 수정·검증 루프를 수행하는지 확인   | `corepack pnpm test:scenario:user`                                                     | `decision=accept`, `ALL_PASS`                    | 네트워크/LLM 불필요, 빠른 회귀 확인 |
| 2    | Codex ChatGPT login + 내장 OAuth proxy UAT | API key 없이 실제 Codex `gpt-5.5`/`xhigh`가 문제 수정 후 하네스 gate를 통과하는지 확인 | `corepack pnpm uat:codex-oauth`                                                        | `status=accepted`, `decision=accept`, `ALL_PASS` | 실제 사용자 환경 대표 경로          |
| 3    | 외부 OSS OAuth proxy 연결 UAT              | `openai-oauth` 등 별도 localhost proxy를 붙여 같은 시나리오를 검증                     | `VIBELOOP_UAT_OAUTH_PROXY_URL=http://127.0.0.1:10531/v1 corepack pnpm uat:codex-oauth` | 동일                                             | 외부 proxy를 직접 띄운 경우         |
| 4    | API-key proxy 보존 UAT                     | API-key 기반 proxy 경로가 필요한 환경의 비교/회귀 확인                                 | `corepack pnpm uat:codex-proxy`                                                        | 동일 또는 credential block                       | 기본 경로 아님                      |
...
```bash
corepack pnpm test:scenario:user
```
...
1. `corepack pnpm install --frozen-lockfile`로 의존성을 맞춘다.
2. `corepack pnpm test:scenario:user`를 실행한다.
...
```bash
npx openai-oauth
VIBELOOP_UAT_OAUTH_PROXY_URL=http://127.0.0.1:10531/v1 corepack pnpm uat:codex-oauth
```
...
| localhost proxy + OpenAI API key auth                        | **blocked** | `corepack pnpm uat:codex-proxy` → `MISSING_PROXY_API_KEY`; 현재 셸에 `VIBELOOP_UAT_OPENAI_API_KEY`/`OPENAI_API_KEY` 없음                              |
| localhost OAuth proxy + ChatGPT login auth                   | **통과**    | `corepack pnpm uat:codex-oauth` → `accept/ALL_PASS`; `gpt-5.5`/`xhigh`, `/v1/responses` 12회 upstream 200, hidden/token 원문 비노출                   |

실제 Codex CLI 실행에서 확인한 환경 보정:

- 2026-06-15 현재 live preflight 복구 기준은 `@openai/codex@0.139.0` / `codex-cli 0.139.0`이다.
- `codex exec`와 login 확인에는 `-c service_tier=fast` override를 사용한다.
...
- live UAT 전에는 `corepack pnpm uat:live-preflight`를 먼저 실행한다. `codex` 실행, ChatGPT login, `gh auth status`, `corepack pnpm` 중 필수 항목이 실패하면 run FAIL이 아니라 blocked(exit 20)로 끝낸다.
